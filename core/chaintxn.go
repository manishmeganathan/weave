...
func (chain *BlockChain) FindTransaction(txnid []byte) (primitives.Transaction, error) {
	...
	return primitives.Transaction{}, fmt.Errorf("transaction does not exist")
}
...
func (chain *BlockChain) SignTransaction(txn *primitives.Transaction, privatekey ecdsa.PrivateKey) {
	...
	prevtxns := make(map[string]primitives.Transaction)
	...
	for _, input := range txn.Inputs {
		...
		utils.HandleErrorLog(err, fmt.Sprintf("could not find transaction associated with txn input - %s", input.ID))
	...
	for inpindex, input := range txncopy.Inputs {
		...
		txncopy.Inputs[inpindex].PublicKey = primitives.PublicKey(pubkeyhash)
		...
		utils.HandleErrorLog(err, "transaction sign failed!")
